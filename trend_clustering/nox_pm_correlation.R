...
correlations = sites %>%
  left_join(data) %>%
  group_by(site) %>%
  filter(sum(complete.cases(nox, pm2.5)) >= 2) %>%  # keep only groups with ≥2 valid pairs
  summarise(correlation = cor(nox, pm2.5, use = "complete.obs")) %>%
  ungroup() %>%
  left_join(clusters)
...
(site_map = ggplot(data = uk_map) +
    geom_sf(fill = "darkseagreen3", color = "white") +
